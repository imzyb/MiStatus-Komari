...
- **详情抽屉**：点击卡片/列表行弹出详情面板，含 2×2 指标卡片 + 延迟历史图表
- **延迟监测**：联通/电信/移动三线折线图，支持 1H/6H/12H/24H 时间范围
- **数字动画**：统计卡片数据刷新时平滑过渡（ease-out 计数器）
- **WebSocket 实时状态**：导航栏连接指示灯（绿=实时 / 红=离线）
- **滚动响应页头**：滚动时自动添加阴影 + 背景透明度变化
- **地区筛选**：下拉选择器，显示国旗 emoji + 中文名 + 服务器数量徽标
