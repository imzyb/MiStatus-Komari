  ...
  function ServerDetailDrawer() {
    ...
    return (
      <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-[#2c2c2c] rounded-t-3xl sm:rounded-3xl shadow-2xl animate-slide-up max-h-[85vh] overflow-y-auto">
          <div className="sticky top-0 bg-white/90 dark:bg-[#2c2c2c]/90 backdrop-blur-xl border-b border-hairline/50 px-5 py-3 flex items-center justify-between z-10 rounded-t-3xl">
          ...
          <div className="p-5 space-y-5">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-muted/40 p-3 space-y-1">
                ...
                <div className="text-lg font-semibold font-mono leading-tight">{cpuP}%</div>
              ...
              <div className="rounded-2xl bg-muted/40 p-3 space-y-1">
                ...
                <div className="text-lg font-semibold font-mono leading-tight">{memP}%</div>
                <div className="h-1 rounded-full bg-muted overflow-hidden">
                  <div className={`h-full rounded-full ${memP >= 90 ? "bg-trading-down" : "bg-trading-up"}`} style={{ width: `${memP}%` }} />
                </div>
                <div className="text-[10px] text-muted-foreground font-mono">{formatBytes(s.memory_used)} / {formatBytes(s.memory_total)}</div>
              </div>
              <div className="rounded-2xl bg-muted/40 p-3 space-y-1">
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  <HardDrive className="h-3 w-3" /> 硬盘
                </div>
                <div className="text-lg font-semibold font-mono leading-tight">{diskP}%</div>
                <div className="h-1 rounded-full bg-muted overflow-hidden">
                  <div className={`h-full rounded-full ${diskP >= 90 ? "bg-trading-down" : "bg-trading-up"}`} style={{ width: `${diskP}%` }} />
                </div>
                <div className="text-[10px] text-muted-foreground font-mono">{formatBytes(s.hdd_used)} / {formatBytes(s.hdd_total)}</div>
              </div>
              <div className="rounded-2xl bg-muted/40 p-3 space-y-1">
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                ...
                <div className="text-sm font-mono leading-relaxed space-y-0.5">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{formatBytes(s.network_in)}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-trading-up">↑</span>
                    <span>{formatBytes(s.network_out)}</span>
                  </div>
                </div>
              </div>
            </div>
