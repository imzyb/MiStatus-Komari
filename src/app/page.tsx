...
import { useThemeSettings, type ThemeSettings } from "@/contexts/theme-settings-context";
import { DashboardErrorBoundary } from "@/components/dashboard/error-boundary";

function SkeletonCard() {
  return (
    <div className="rounded-2xl bg-card shadow-sm h-[300px]">
...
function SettingsAwareSkeleton({ settings }: { settings: ThemeSettings }) {
  return (
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-5 space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className={`grid gap-3 server-grid server-grid-cols-${settings.cardColumns}`}>
          {Array.from({ length: settings.cardColumns * 2 }).map((_, i) => (
            <SkeletonCard key={i} />
          ))}
        </div>
      </div>
...
function MinimalSkeleton() {
  return (
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 animate-fade-in">
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          <div className="h-10 w-10 rounded-xl skeleton" />
          <div className="flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full skeleton" />
            <span className="h-1.5 w-1.5 rounded-full skeleton" />
            <span className="h-1.5 w-1.5 rounded-full skeleton" />
          </div>
        </div>
  ...
  if (!data && isLoading) {
    return (
      <div className="flex flex-col min-h-screen">
        <ClientNavbar />
        <main className="flex-1 flex flex-col">
          <SettingsAwareSkeleton settings={settings} />
        </main>
        <ServerFooter />
      </div>
    );
